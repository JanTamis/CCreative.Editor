...
namespace CCreative.Data
{
	public class XML
	{
		...
		public int GetInt(string name, int defaultValue = default)
		{
			...
			if (attribute is not null && Int32.TryParse(attribute.Value, out var result))
			{
				return result;
			}
		...
		public float GetFloat(string name, float defaultValue = default)
		{
			...
			if (attribute is not null && Single.TryParse(attribute.Value, out var result))
			{
				return result;
			}
		...
		{
			if (Single.TryParse(GetContent(), out float result))
			{
				return result;
			}
